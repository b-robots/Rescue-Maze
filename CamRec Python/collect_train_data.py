...
out_l = cv.VideoWriter('left.avi', cv.VideoWriter_fourcc('M','J','P','G'), 10, (img_size, img_size), False)
out_r = cv.VideoWriter('right.avi', cv.VideoWriter_fourcc('M','J','P','G'), 10, (img_size, img_size), False)
...
last_left = np.zeros((img_size, img_size), dtype=np.uint8)
last_right = np.zeros((img_size, img_size), dtype=np.uint8)
...
while True:
    ...
    right_img = get_undist_roi(right_img, map1, map2, True)
